---
title: "UK election 2019 study: Flips and swings"
author: "Kevin Horan"
toc: true
format:
  html:
...
Explains con-swing in terms of PC1, PC2, and a smooth which captures effects which have not been accounted for in the model and are correlated with geographic neighbourhood structure.

PC1 and PC2 both show significant negative associations with con_swing, with the effect of PC2 being three times larger. On a global scale, an increase in `con_swing` is associated with the PC1 effect of higher `over65, lower density, student` and `born elsewhere`, and the much stronger PC2 effect of lower `deprived` and `low-qual`.

```{r}
model1 <- gam(con_swing ~ PC1 + PC2 +
                s(constituency_name,bs='mrf',xt=list(nb=nlistconst),k=275),
...
model1df <- predict(model1,newdata = temp,type = "terms") |> 
  as.data.frame() |> 
...
Looking at the unaccounted for geographic effects, these PC1 and PC2 values are countered by negative effects on `con_swing` in the general `West` and `South West`, and higher in the `East` and `North East`. These effects are particularly negative around Liverpool and Bradford. They are much higher in constituencies such as Scunthorpe and Grimsby in `Yorkshire and the Humber`.

`London` is also divided between areas of high positive and high negative geographic effects.

```{r}
tmap_mode("plot")

# make separate London map in addition to full map
maplondon <- tm_shape(model1df |> filter(region == "London")) + 
  tm_polygons("s(constituency_name)", lwd=0.5, palette="RdBu", id="constituency_name", breaks = seq(-6,6,by=1)) + 
  tm_layout(legend.show=FALSE)

mapgb <- tm_shape(model1df) + 
  tm_fill(col="s(constituency_name)", style='cont', palette="RdBu", id="constituency_name") + 
  tm_shape(model1df) + tm_borders(lwd=0.1) + 
  tm_shape(regions) + tm_borders(lwd=1)
...
tm_shape(model1df) + 
  tm_polygons("s(constituency_name)", lwd=0.5, palette="RdBu", id="constituency_name")
```

Model 1 leaves uncorrelated residuals with a chequerboard pattern.

```{r}
model1df$residuals <- model1$residuals
df_moran_model1 <- moran.test(model1df$residuals, df_wts, zero.policy = TRUE)

ggplot() + 
  geom_sf(data=gb, lwd=0.5) +
  geom_sf(data = model1df, aes(fill = residuals), lwd=0.05, colour="black") + 
  geom_sf(data = regions, lwd = 0.5, fill=NA) + 
  scale_fill_gradient2(high = "darkblue", low="darkred", mid = "white", midpoint = 0) + 
  labs(title = "Model 1: Residuals",
       subtitle = paste0("Moran's-I = ",round(df_moran_model1$estimate[1],2))) + 
  theme_bw() + 
  theme(legend.position="right")
```
...
#Model 2 is the same as model 1 but allows interaction between each PC and the category of `rur_urb` to which it belongs.


model2 <- gam(con_swing ~ PC1:rur_urb + PC2:rur_urb +
                s(constituency_name,bs='mrf',xt=list(nb=nlistconst),k=275),
              data=df, family=gaussian, weight=total_vote_19, method='REML')
summary(model2)
...
model2df <- predict(model2,newdata = temp,type = "terms") |> 
  as.data.frame() |> 
  cbind(df |> select(constituency_name,
                     geometry,
                     rur_urb,
...
```{r, echo=FALSE, eval=FALSE}
#The table shows the effect which PC1 or PC2 have on con_swing as a results of being in each different `rur_urb` setting


tab2 <- model2df[,c(1,2,5)] |> 
  st_drop_geometry() |> 
  group_by(rur_urb) |> 
  summarise(`PC1:rur_urb` = mean(`PC1:rur_urb`),
            `rur_urb:PC2` = mean(`rur_urb:PC2`)) |> 
  pivot_longer(cols = c(`PC1:rur_urb`,`rur_urb:PC2`),
               names_to = "name",
               values_to = "coefficient") |> 
  separate(name, c("name1","name2"),sep = ":") |> 
  mutate(PC = case_when(name1=="PC1" ~ "PC1",
                        name2=="PC2" ~ "PC2"))

ggplot(tab2) + 
  geom_tile(aes(x=rur_urb, y=PC, fill=coefficient), colour="black") + 
  geom_text(aes(x=rur_urb, y=PC, label=round(coefficient,2))) + 
...
```{r, echo=FALSE, eval=FALSE}
#The geographic component is similar to model 0 but slightly less pronounced as we are also controlling for `rur_urb` status.


tmap_mode("plot")

# make separate London map in addition to full map
maplondon <- tm_shape(model2df |> filter(region == "London")) + 
  tm_polygons("s(constituency_name)", lwd=0.5, palette="RdBu", id="constituency_name") + 
  tm_layout(legend.show=FALSE)

mapgb<- tm_shape(model2df) + tm_polygons() + 
  tm_shape(model2df) + 
  tm_fill(col="s(constituency_name)", style='cont', palette="RdBu", id="constituency_name") + 
  tm_shape(model2df) + tm_borders(lwd=0.1)
...
# Focussing in on PC2 as it has the much larger effect and a larger variance of effect by `rur_urb`. Try to extract effect of PC2 as a results of `rur_urb`
# 
# GAM with random effects


model3 <- gam(con_swing ~ PC1 + PC2 +
                s(rur_urb, bs="re") + 
                s(rur_urb, PC2, bs="re") + 
                s(constituency_name,bs='mrf',xt=list(nb=nlistconst),k=275),
              data=df, family=gaussian, weight=total_vote_19, method='REML')
...
model3df <- predict(model3,newdata = temp,type = "terms") |> 
  as.data.frame() |>  
  cbind(df |> select(constituency_name,
                     geometry,
                     rur_urb,
...
tab3 <- model3df[,c(1:4,7)] |> 
  st_drop_geometry() |> 
  group_by(rur_urb) |> 
  summarise(PC1 = mean(PC1),
            PC2 = mean(PC2),
            `s(rur_urb)` = mean(`s(rur_urb)`),
            `s(rur_urb,PC2)` = mean(`s(rur_urb,PC2)`)) |> 
  pivot_longer(cols = c(`s(rur_urb,PC2)`,`s(rur_urb)`),
               names_to = "name",
               values_to = "coefficient")

ggplot(tab3) + 
  geom_tile(aes(x=rur_urb, y=name, fill=coefficient), colour="black") + 
  geom_text(aes(x=rur_urb, y=name, label=round(coefficient,2))) + 
  scale_fill_gradient2(low="darkred",mid = "white",high = "darkblue",midpoint = 0) + 
  theme(panel.background = element_rect(fill = "darkgrey",
                                colour = "black",
                                size = 0.5, linetype = "solid"), 
...
# row1: being in small places good for conswing, big places bad
# row2: means increase in poverty good for conswing in small places, bad in cities
# (when PC2 is being multiplied, red is good for conswing)
...
model4 <- gam(con_swing ~ PC1 + PC2 +
                s(rur_urb,region, bs="re") + 
                s(rur_urb, PC2, bs="re") + 
                s(constituency_name,bs='mrf',xt=list(nb=nlistconst),k=275),
              data=df, family=gaussian, weight=total_vote_19, method='REML')
summary(model4)
...
model4df <- predict(model4,newdata = temp,type = "terms") |> 
  as.data.frame() |>  
  cbind(df |> select(constituency_name,
                     geometry,
                     rur_urb,
...
tab4 <- model4df[,c(1:4,7,8)] |> 
  st_drop_geometry() |> 
  group_by(region,rur_urb) |> 
  summarise(PC1 = mean(PC1),
            PC2 = mean(PC2),
            `s(rur_urb,region)` = mean(`s(rur_urb,region)`),
            `s(rur_urb,PC2)` = mean(`s(rur_urb,PC2)`)) |> 
  pivot_longer(cols = c(`s(rur_urb,PC2)`,`s(rur_urb,region)`),
               names_to = "name",
               values_to = "coefficient")

ggplot(tab4) + 
  geom_tile(aes(x=rur_urb, y=region, fill=coefficient), colour="black") + 
  geom_text(aes(x=rur_urb, y=region, label=round(coefficient,2))) + 
  scale_fill_gradient2(low="darkred",mid = "white",high = "darkblue",midpoint = 0) + 
  theme(panel.background = element_rect(fill = "darkgrey",
                                colour = "black",
                                size = 0.5, linetype = "solid"), 
  ...
  scale_y_discrete(guide = guide_axis(n.dodge = 1.5)) + 
  facet_wrap(~name)
# row1: being in small places good for con_swing, big places bad
# row2: means increase in poverty good for con_swing in small places, bad in cities
...
model5 <- gam(con_swing ~ PC1 + PC2 +
                s(rur_urb, PC2, region, bs="re") + 
                s(constituency_name,bs='mrf',xt=list(nb=nlistconst),k=275),
              data=df, family=gaussian, weight=total_vote_19, method='REML')
summary(model5)
...
model5df <- predict(model5,newdata = temp,type = "terms") |> 
  as.data.frame() |>  
  cbind(df |> select(constituency_name,
                     geometry,
                     rur_urb,
...
```{r, fig.width=12, echo=FALSE, eval=FALSE}
tab5 <- model5df[,c(1:4,6,7)] |> 
  st_drop_geometry() |> 
  group_by(region,rur_urb) |> 
  summarise(PC1 = mean(PC1),
            PC2 = mean(PC2),
            `s(rur_urb,PC2,region)` = mean(`s(rur_urb,PC2,region)`)) |> 
  pivot_longer(cols = c(`s(rur_urb,PC2,region)`),
               names_to = "name",
               values_to = "coefficient")
...
```{r, echo=FALSE, eval=FALSE}
#Just geography: model5 Controls for PC1, PC2, region, rur_urb


tmap_mode("plot")

# make separate London map in addition to full map
maplondon <- tm_shape(model5df |> filter(region == "London")) + 
  tm_polygons("s(constituency_name)", lwd=0.5, palette="RdBu", id="constituency_name") + 
  tm_layout(legend.show=FALSE)

mapgb<- tm_shape(model5df) + tm_polygons() + 
  tm_shape(model5df) + 
  tm_fill(col="s(constituency_name)", style='cont', palette="RdBu", id="constituency_name") + 
  tm_shape(model5df) + tm_borders(lwd=0.1)
...
model6 <- gam(con_swing ~ PC1 + PC2 +
                s(rur_urb, region, bs="re") + 
                s(constituency_name,bs='mrf',xt=list(nb=nlistconst),k=100, by = PC2) + 
                s(constituency_name,bs='mrf',xt=list(nb=nlistconst),k=275),
              data=df, family=gaussian, weight=total_vote_19, method='REML')
summary(model6)
...
model6df <- predict(model6,newdata = temp,type = "terms") |> 
  as.data.frame() |>  
  cbind(df |> select(constituency_name,
                     geometry,
                     rur_urb,
...
tab6 <- model6df[,c(1:8)] |> 
  st_drop_geometry() |> 
  group_by(region,rur_urb) |> 
  summarise(PC1 = first(PC1),
            PC2 = first(PC2),
            `s(rur_urb,region)` = first(`s(rur_urb,region)`)) |> 
  pivot_longer(cols = c(`s(rur_urb,region)`),
               names_to = "name",
               values_to = "coefficient")
...
# make separate London map in addition to full map
maplondon <- tm_shape(model6df |> filter(region == "London")) + 
  tm_polygons("s(constituency_name):PC2", lwd=0.5, palette="-RdBu", id="constituency_name") + 
  tm_layout(legend.show=FALSE)

mapgb<- tm_shape(model6df) + tm_polygons() + 
  tm_shape(model6df) + 
  tm_fill(col="s(constituency_name):PC2", style='cont', palette="RdBu", id="constituency_name") + 
  tm_shape(regions) + tm_borders(lwd = 1) + 
  tm_shape(model6df) + tm_borders(lwd=0.1)
...
```{r, echo=FALSE, eval=FALSE}
tableAIC <- tibble(Model = c("model1","model2","model3","model4","model5","model6"),
                   Formula = c(model1$formula, model2$formula,model3$formula,model4$formula,model5$formula,model6$formula),
                   AIC = c(model1$aic,model2$aic,model3$aic,model4$aic,model5$aic,model6$aic),
                   relativeAIC = AIC/max(AIC)) |> 
  mutate(Formula = paste0(substring(Formula, 1, nchar(Formula)-72)," + spatial smooth")) |> 
  arrange(desc(relativeAIC))
kable(tableAIC, caption = "Model Comparison by descending AIC") %>%
...
model7 <- gam(con_swing ~ PC1 + PC2 + 
                s(constituency_name,bs='mrf',xt=list(nb=nlistconst),k=100, by = PC2) + 
                s(constituency_name,bs='mrf',xt=list(nb=nlistconst),k=300),
              data=df, family=gaussian, weight=total_vote_19, method='REML')
summary(model7)
...
model7df <- predict(model7,newdata = temp,type = "terms") |> 
  as.data.frame() |>  
  cbind(df |> select(constituency_name,
                     geometry,
                     rur_urb,
...
Model 7 leaves uncorrelated residuals with a chequerboard pattern.

```{r}
model7df$residuals <- model7$residuals
df_moran_model7 <- moran.test(model7df$residuals, df_wts, zero.policy = TRUE)

ggplot() + 
  geom_sf(data=gb, lwd=0.5) +
  geom_sf(data = model7df, aes(fill = residuals), lwd=0.05, colour="black") + 
  geom_sf(data = regions, lwd = 0.5, fill=NA) + 
  scale_fill_gradient2(high = "darkblue", low="darkred", mid = "white", midpoint = 0) + 
  labs(title = "Model 7: Residuals",
       subtitle = paste0("Moran's-I = ",round(df_moran_model7$estimate[1],2))) + 
  theme_bw() + 
  theme(legend.position="right")
```
...
```{r, fig.height=8}
ggplot(model7df) + 
  geom_boxplot(aes(x=rur_urb, y = `s(constituency_name):PC2`, fill = rur_urb)) + 
  scale_fill_brewer(palette = "Paired") +
  ...
  theme_bw() +
  theme(legend.position="bottom") + 
  labs(title = "Coefficient: geographic by PC2",
       subtitle = "To the left is helpful to `con_swing`")

```

```{r, fig.height=8}
ggplot(model7df) + 
  geom_boxplot(aes(x=region, y = `s(constituency_name):PC2`, fill = region)) + 
  scale_fill_brewer(palette = "Paired") +
  geom_hline(yintercept = 0, colour="red") + 
  facet_wrap(~rur_urb, ncol = 3) + 
  coord_flip() + 
  theme_bw() +
  theme(legend.position="bottom") + 
  labs(title = "Coefficient: geographic by PC2",
...
tab7 <- model7df[,c(1:8)] |> 
  ...
  summarise(PC1 = first(PC1),
            PC2 = first(PC2),
            `s(constituency_name):PC2` = mean(`s(constituency_name):PC2`)) |> 
  pivot_longer(cols = c(`s(constituency_name):PC2`),
               names_to = "name",
               values_to = "coefficient")
...
# make separate London map in addition to full map
maplondon <- tm_shape(model7df |> filter(region == "London")) + 
  tm_polygons("s(constituency_name):PC2", lwd=0.5, palette="RdBu", id="constituency_name", breaks = seq(-1,1.5,by=0.1)) + 
  tm_layout(legend.show=FALSE)

mapgb<- tm_shape(model7df) + tm_polygons() + 
  tm_shape(model7df) + 
  tm_fill(col="s(constituency_name):PC2", style='cont', palette="RdBu", id="constituency_name") + 
  tm_shape(regions) + tm_borders(lwd = 1) + 
  tm_shape(model7df) + tm_borders(lwd=0.1)
...
```{r}
tableAIC <- tibble(Model = c("model1","model7"),
                   Formula = c(model1$formula,model7$formula),
                   AIC = c(model1$aic,model7$aic),
                   relativeAIC = AIC/max(AIC)) |> 
  mutate(Formula = paste0(substring(Formula, 1, nchar(Formula)-72)," + spatial smooth")) |> 
  arrange(desc(relativeAIC))
kable(tableAIC, caption = "Model Comparison by descending AIC") %>%
...
meantab7 <- tab7 |> group_by(rur_urb) |> summarise(x=mean(coefficient))
ggplot(meantab7) + 
  geom_tile(aes(x=rur_urb, y=0, fill=x), colour="black") + 
  geom_text(aes(x=rur_urb, y=0, label=round(x,2)), size=10) + 
  scale_fill_gradient2(low="darkred",mid = "white",high = "darkblue",midpoint = 0, limits=c(-0.7,1.2)) +
  theme_bw() + 
  theme(legend.position = "none", 
...
#### Map of model7 `fill=`s(constituency_name)\`\`
...
# make separate London map in addition to full map
maplondon <- tm_shape(model7df |> filter(region == "London")) + 
  tm_polygons("s(constituency_name)", lwd=0.5, palette="RdBu", id="constituency_name", breaks = seq(-5,5,by=0.1)) + 
  tm_layout(legend.show=FALSE)
...
```{r, fig.width=12, fig.height=12}
# library(rayshader)
# plt <- ggplot(model7df,aes(fill=`s(constituency_name)`), lwd=0.1) + 
#   geom_sf(show.legend = FALSE) + 
#   scale_fill_gradient2(high = "darkblue", low="darkred", mid = "white", midpoint = 0) + 
#   theme_bw()
# plot_gg(plt,fov = 30, theta = -40, zoom = 0.75, phi = 50, windowsize = c(1000, 800))
# Sys.sleep(1)
...
model7df$PC2total <- model7df$`s(constituency_name):PC2` + model7df$PC2
# make separate London map in addition to full map
maplondon <- tm_shape(model7df |> filter(region == "London")) + 
  tm_polygons("PC2total", lwd=0.5, palette="RdBu", id="constituency_name", breaks = seq(-2.5,0.5,by=0.1)) + 
  tm_layout(legend.show=FALSE)

mapgb<- tm_shape(model7df) + tm_polygons() + 
  tm_shape(model7df) + 
  tm_fill(col="PC2total", style='cont', palette="RdBu", id="constituency_name") + 
  tm_shape(regions) + tm_borders(lwd = 1) + 
  tm_shape(model7df) + tm_borders(lwd=0.1)
...
model7dfhex <- model7df |> 
  st_drop_geometry() |> 
  mutate(geometry = df_hex$geometry) |> 
  st_as_sf()

# make separate London map in addition to full map
maplondon <- tm_shape(model7dfhex |> filter(region == "London")) + 
  tm_polygons("PC2total", lwd=0.5, palette="RdBu", id="constituency_name", breaks = seq(-2.5,0.5,by=0.1)) + 
  tm_layout(legend.show=FALSE)

mapgb<- tm_shape(model7dfhex) + tm_polygons(col="PC2total", style='cont', palette="RdBu", id="constituency_name") + 
  #tm_shape(model7df) + 
  #tm_fill(col="PC2total", style='cont', palette="RdBu", id="constituency_name") + 
  #tm_shape(regions) + tm_borders(lwd = 1) + 
  tm_shape(model7dfhex) + tm_borders(lwd=0.1)
...
#### Standardise \$total and \$s(con): flipping one of the colour palettes for comparison purposes

```{r}
tmap_mode("plot")

model7df$std_total <- (model7df$PC2total-min(model7df$PC2total, na.rm=T))/
                         (max(model7df$PC2total, na.rm=T)-min(model7df$PC2total, na.rm=T))
model7df$int_total <- (model7df$`s(constituency_name)`-min(model7df$`s(constituency_name)`, na.rm=T))/
                         (max(model7df$`s(constituency_name)`, na.rm=T)-min(model7df$`s(constituency_name)`, na.rm=T))

tm_shape(model7df) + 
  tm_polygons("std_total", lwd=0.5, palette="-RdBu", id="constituency_name")

tm_shape(model7df) + 
  tm_polygons("int_total", lwd=0.5, palette="RdBu", id="constituency_name")
```
...
#### Comparing `student` variable to residuals model7

```{r}
plot(df$student,model7df$residuals)
plot(df$PC3,model7df$residuals)
```

### df saved at this point

```{r}
saveRDS(df, here("df_withtext.rds"))
```
...
<!-- # dummy encode the regions and rur_urb -->

<!-- df_tree <- df |> -->

<!--   mutate(east = case_when(region == "East" ~ 1, TRUE ~0), -->

<!--          emid = case_when(region == "East Midlands" ~ 1, TRUE ~0), -->

<!--          lon = case_when(region == "London" ~ 1, TRUE ~0), -->

<!--          neast = case_when(region == "North East" ~ 1, TRUE ~0), -->

<!--          nwest = case_when(region == "North West" ~ 1, TRUE ~0), -->

<!--          seast = case_when(region == "South East" ~ 1, TRUE ~0), -->

<!--          swest = case_when(region == "South West" ~ 1, TRUE ~0), -->

<!--          wales = case_when(region == "Wales" ~ 1, TRUE ~0), -->

<!--          wmid = case_when(region == "West Midlands" ~ 1, TRUE ~0), -->

<!--          yhumb = case_when(region == "Yorkshire and The Humber" ~ 1, TRUE ~0), -->

<!--          vill = case_when(rur_urb == "Village or smaller" ~ 1, TRUE ~0), -->

<!--          stown = case_when(rur_urb == "Small Town" ~ 1, TRUE ~0), -->

<!--          mtown = case_when(rur_urb == "Medium Town" ~ 1, TRUE ~0), -->

<!--          ltown = case_when(rur_urb == "Large Town" ~ 1, TRUE ~0), -->

<!--          cty = case_when(rur_urb == "Other City" ~ 1, TRUE ~0), -->

<!--          corecty = case_when(rur_urb == "Core City (outside London)" ~ 1, TRUE ~0)) |> -->

<!--   select(con_swing,over65,low_qual,deprived,student,born_elsewhere,density, -->

<!--          PC1,PC2, -->

<!--          east,emid,lon,neast,nwest,seast,swest,wales,wmid,yhumb, -->

<!--          vill,stown,mtown,ltown,cty,corecty) |> -->
...
<!-- ggplot(melt(df_tree[,1:6]),aes(x=value, fill=variable)) + -->
...
<!--   geom_density(aes(x=low_qual), fill="pink") + -->

<!--   geom_vline(xintercept = 39, colour="red", size=1) + -->
...
<!--   geom_vline(xintercept = 31, colour="red", size=1) + -->

<!--   theme_bw() -->

<!-- t2.2 <- ggplot(df_tree) + -->

<!--   geom_density(aes(x=student), fill="orange") + -->

<!--   geom_vline(xintercept = 4.7, colour="red", size=1) + -->
...
<!--   geom_vline(xintercept = 35, colour="red", size=1) + -->

<!--   theme_bw() -->

<!-- t3.2 <- ggplot(df_tree) + -->

<!--   geom_density(aes(x=over65), fill="grey") + -->

<!--   geom_vline(xintercept = 11, colour="red", size=1) + -->
...
<!--   geom_vline(xintercept = 44, colour="red", size=1) + -->

<!--   theme_bw() -->

<!-- plot_grid(NULL,t1,NULL,NULL, -->

<!--           t2.1,NULL,t2.2,NULL, -->

<!--           t3.1,NULL,t3.2,t3.3, nrow = 3) -->

<!-- ``` -->

<!-- #### MAE scores: Model7 vs tree -->

<!-- ```{r} -->

<!-- tree_est <- predict(d.tree1,df_tree) -->

<!-- mae_tree <- sum(abs(tree_est - df_tree$con_swing)) / nrow(df_tree) -->

<!-- m7_est <- predict(model7, df) -->

<!-- mae_m7 <- sum(abs(m7_est - df$con_swing)) / nrow(df) -->
...
<!-- reglist <- c("east","emid","lon","neast","nwest","seast","swest","wales","wmid","yhumb") -->

<!-- par(mfrow=c(1,2)) -->

<!-- for (reg in reglist){ -->

<!-- d.tree_reg <- rpart(con_swing ~ -->
...
<!--                 data = df_tree |> filter(eval(parse(text = reg)) == "1"), method = 'anova', -->
...
<!-- urblist <- c("vill","stown","mtown","ltown","cty","corecty") -->

<!-- par(mfrow=c(1,2)) -->

<!-- for (urb in urblist){ -->

<!-- d.tree_urb <- rpart(con_swing ~ -->
...
<!--                 data = df_tree |> filter(eval(parse(text = urb)) == "1"), method = 'anova', -->
...
<!-- df_ngrid <- cbind(df_ngrid,coords) -->

<!-- d.tree_xy <- rpart(con_swing ~ -->
...
<!-- rpart.plot(d.tree_xy, -->

<!--            box.palette = "RdBu") -->

<!-- tree_est <- predict(d.tree_xy,df_ngrid) -->

<!-- mae_tree <- sum(abs(tree_est - df_ngrid$con_swing)) / nrow(df_ngrid) -->

<!-- ``` -->

<!-- #### Loop different rotations of XY and compare MAE -->

<!-- Display of rotating by 1/25th of pi/2 -->

<!-- ```{r, fig.width=8,fig.height=8} -->

<!-- error_df <- data.frame(angle=seq(0,pi/2,pi/2/25), -->

<!--                        error=rep(0,26)) -->

<!-- maplist <- list() -->

<!-- for (i in 1:(nrow(error_df)-1)){ -->

<!-- temprot <- coords |> -->

<!--   Rotation(error_df[i,1]) |> -->

<!--   data.frame() -->

<!-- maplist[[i]] <- ggplot(temprot) + -->
...
<!-- Now rotate by 1/1000 of pi/2 each time and calculate MAE -->

<!-- ```{r} -->

<!-- error_df <- data.frame(angle=seq(0,pi/2,pi/2/1000), -->
...
<!--   select(con_swing,over65,low_qual,deprived,student,density,born_elsewhere) |> -->
...
<!-- d.tree_loop <- rpart(con_swing ~ ., -->

<!--                 data = zzz, method = 'anova', -->
...
<!-- tree_est <- predict(d.tree_loop,zzz) -->
...
<!--   labs(title = paste0("MAE of regression tree: min at \u03c0/",round(error_df[which.min(error_df$error),1],3)), -->
...
<!-- c1 <- ggplot(coords)+geom_point(aes(x=X,y=Y)) + -->

<!--   labs(title = "No rotation") + -->

<!--   theme_bw() -->

<!-- c2 <- ggplot(minMAEdf)+geom_point(aes(x=X1,y=X2)) + -->

<!--   labs(title = "Rotation of 0.229 for minimum MAE") + -->
...
<!-- #### Boosted regression trees (with bagging) -->
...
<!--   mutate(Xrot = minMAEdf$X1, -->

<!--          Yrot = minMAEdf$X2) -->

<!-- formula <- formula(con_swing ~ -->
...
<!-- swing_split <- initial_split(df_tree, prop = .7) -->
...
<!-- perf_gbm1 = gbm.perf(gbm.fit, method = "cv") -->

<!-- # summarize model -->

<!-- swingEffects <- tibble::as_tibble(gbm::summary.gbm(gbm.fit, -->

<!--                                          plotit = FALSE)) -->

<!-- swingEffects %>% utils::head() -->
...
<!--   ggtitle("Ordered Drivers of con_swing") -->
...
<!-- p_list[[z]] <- plot.gbm(gbm.fit, i.var = c(i,j), col=col.pal) -->
...
<!--   p_list2[[z]] <- plot.gbm(gbm.fit, i.var = i, lwd=3) -->
...
<!-- plot_grid(p_list2[[1]],p_list[[2]],p_list[[3]],p_list[[4]],p_list[[5]],p_list[[6]],p_list[[7]],p_list[[8]],nrow = 2) -->

<!-- plot_grid(p_list[[9]],p_list2[[2]],p_list[[11]],p_list[[12]],p_list[[13]],p_list[[14]],p_list[[15]],p_list[[16]],nrow = 2) -->

<!-- plot_grid(p_list[[17]],p_list[[18]],p_list2[[3]],p_list[[20]],p_list[[21]],p_list[[22]],p_list[[23]],p_list[[24]],nrow = 2) -->

<!-- plot_grid(p_list[[25]],p_list[[26]],p_list[[27]],p_list2[[4]],p_list[[29]],p_list[[30]],p_list[[31]],p_list[[32]],nrow = 2) -->

<!-- plot_grid(p_list[[33]],p_list[[34]],p_list[[35]],p_list[[36]],p_list2[[5]],p_list[[38]],p_list[[39]],p_list[[40]],nrow = 2) -->

<!-- plot_grid(p_list[[41]],p_list[[42]],p_list[[43]],p_list[[44]],p_list[[45]],p_list2[[6]],p_list[[47]],p_list[[48]],nrow = 2) -->

<!-- plot_grid(p_list[[49]],p_list[[50]],p_list[[51]],p_list[[52]],p_list[[53]],p_list[[54]],p_list2[[7]],p_list[[56]],nrow = 2) -->

<!-- plot_grid(p_list[[57]],p_list[[58]],p_list[[59]],p_list[[60]],p_list[[61]],p_list[[62]],p_list[[63]],p_list2[[8]],nrow = 2) -->

<!-- ``` -->

<!-- ```{r, fig.width=12, fig.height=8, echo=FALSE, eval=FALSE} -->

<!-- # for only six variables when not using X and Y as covariates -->
...
<!-- plot_grid(p_list2[[1]],p_list[[2]],p_list[[3]],p_list[[4]],p_list[[5]],p_list[[6]],nrow = 2) -->

<!-- plot_grid(p_list[[7]],p_list2[[2]],p_list[[9]],p_list[[10]],p_list[[11]],p_list[[12]],nrow = 2) -->

<!-- plot_grid(p_list[[13]],p_list[[14]],p_list2[[3]],p_list[[16]],p_list[[17]],p_list[[18]],nrow = 2) -->

<!-- plot_grid(p_list[[19]],p_list[[20]],p_list[[21]],p_list2[[4]],p_list[[23]],p_list[[24]],nrow = 2) -->

<!-- plot_grid(p_list[[25]],p_list[[26]],p_list[[27]],p_list[[28]],p_list2[[5]],p_list[[30]],nrow = 2) -->

<!-- plot_grid(p_list[[31]],p_list[[32]],p_list[[33]],p_list[[34]],p_list[[35]],p_list2[[6]],nrow = 2) -->

<!-- ``` -->

<!-- ### Make quintile categories of covariates -->
...
<!--   mutate(across(c(over65,low_qual,deprived,student,born_elsewhere,density), ~quant_groups(.x,5))) -->

<!-- levels(df_cat$over65) <- c("Lowest","Low","Mid","High","Highest") -->

<!-- levels(df_cat$low_qual) <- c("Lowest","Low","Mid","High","Highest") -->

<!-- levels(df_cat$deprived) <- c("Lowest","Low","Mid","High","Highest") -->

<!-- levels(df_cat$student) <- c("Lowest","Low","Mid","High","Highest") -->

<!-- levels(df_cat$born_elsewhere) <- c("Lowest","Low","Mid","High","Highest") -->

<!-- levels(df_cat$density) <- c("Lowest","Low","Mid","High","Highest") -->
...
<!-- # dependent.variable.name <- "con_swing" -->

<!-- # predictor.variable.names <- c("over65","low_qual","deprived","student","born_elsewhere","density") -->
...
<!-- # distance.matrix <- dist(xy,method = "euclidean") -->
...
<!-- df_rf <- df_tree[,c(1:7,26,27)] -->

<!-- set.seed(4543) -->

<!-- rf.fit <- randomForest(con_swing ~ ., data=df_rf, ntree=1000, mtry=6, -->
...
<!-- rf_est <- rf.fit$predicted -->

<!-- mae_rf <- sum(abs(rf_est - df_tree$con_swing)) / nrow(df_tree) -->
